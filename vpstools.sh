...
Configure_BBR_OV(){
	cat /dev/net/tun
	echo "若显示 ${Green_font_prefix} cat: /dev/net/tun: File descriptor in bad state ${Font_color_suffix}，则表示你的VPS支持此脚本，否则请发工单开启${Green_font_prefix} TUN/TAP支持 ${Font_color_suffix} " && echo
	echo "若VPS支持请继续 [y/N]" && echo
	if [[ ${unyn} == [Nn] ]]; then
		echo && echo -e "${Info} 已取消..." && exit 1
		else
		echo -e "————————
...
echo && echo -e " VPS工具包 一键安装管理脚本 ${Red_font_prefix}V 1.0.0 19-5-12${Font_color_suffix}
  -- Toyo | ChennHaoo --
  
  
 ${Green_font_prefix} 0.${Font_color_suffix} 安装常用依赖
 ${Green_font_prefix} 1.${Font_color_suffix} 更新软件源
 ${Green_font_prefix} 2.${Font_color_suffix} 更新系统（慎重）
 ${Green_font_prefix} 3.${Font_color_suffix} 修改系统时间
————————————
 ${Green_font_prefix} 4.${Font_color_suffix} 修改 SSH端口（宝塔用户请勿使用）
 ${Green_font_prefix} 5.${Font_color_suffix} 配置 KVM BBR
 ${Green_font_prefix} 6.${Font_color_suffix} 配置 OpenVZ BBR
 ${Green_font_prefix} 7.${Font_color_suffix} 修改系统文件句柄数（慎用）
 ${Green_font_prefix} 8.${Font_color_suffix} 一键封禁 BT/PT/SPAM (iptables)
 ${Green_font_prefix} 9.${Font_color_suffix} 一键解封 BT/PT/SPAM (iptables)
————————————" && echo
echo
